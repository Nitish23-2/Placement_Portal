...
export async function GET() {
  ...
  let query = auth.supabase.from("applications").select("id, status, applied_at, students!inner(enrollment_no, branch, batch_year), drives(title, companies(name))").order("applied_at", { ascending: false }).limit(100);

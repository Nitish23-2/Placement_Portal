...
export async function GET(request: NextRequest) {
  ...
  let query = auth.supabase
    .from("students")
    .select("id, enrollment_no, branch, batch_year, cgpa, active_backlogs, profile_complete, archived, resume_url, users(full_name, email)", {
      count: "exact",
    })
    .order("branch")
    .order("enrollment_no")
  ...
  if (auth.role === "faculty") {
    ...
    query = query.or(`branch.eq.${normalized},branch.ilike.%${user.branch_scope}%`);
  }

  const branch = params.get("branch")?.trim();
  if (branch && auth.role === "admin") {
    const normalizedBranch = normalizeBranchCode(branch);
    query = query.or(`branch.eq.${normalizedBranch},branch.ilike.%${branch}%`);
  }

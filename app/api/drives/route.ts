...
export async function GET(request: NextRequest) {
  ...
  let query = supabase.from("drives").select("*, companies(name, sector)", { count: "exact" }).eq("status", "published").order("apply_deadline", { ascending: true, nullsFirst: false }).range(from, from + limit - 1);

...
export async function GET(request: NextRequest) {
  ...
  let query = supabase.from("drives").select("*, companies(name, sector)", { count: "exact" }).eq("status", requestedStatus || "published").order("apply_deadline", { ascending: true, nullsFirst: false }).range(from, from + limit - 1);
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await auth.supabase.from("drives").insert({ ...parsed.data, created_by: auth.user.id, status: "draft" }).select("*").single();
  if (error) return errorResponse(error.message, "DATABASE_ERROR", 500);

...
export default async function DrivesPage() {
  ...
  return <main className="portal-page"><header className="portal-header"><Link className="brand" href="/dashboard"><span className="brand-mark">PP</span><span><strong>Placement Portal</strong><small>GBPUAT Pantnagar</small></span></Link><nav className="portal-nav"><Link href="/notices">Notices</Link><Link href="/applications">Applications</Link><Link href="/profile">Profile</Link></nav></header><section className="listing-intro"><p className="eyebrow">Open opportunities</p><h1>Every published drive, in one view.</h1><p className="dashboard-copy">Eligibility notes are shown for your own assessment. The portal does not hide drives based on branch, CGPA, batch, or backlogs.</p></section><section className="drive-list" aria-label="Published placement drives">{drives.length ? drives.map((drive) => <Link className="drive-item" href={`/drives/${drive.id}`} key={drive.id}><div><span className="card-kicker">{drive.companies?.name ?? "Company"} {drive.companies?.sector ? ` / ${drive.companies.sector}` : ""}</span><h2>{drive.title}</h2><p>{drive.description ?? "Role details will be shared by the placement cell."}</p></div><div className="drive-meta"><strong>{formatCtc(drive.ctc_min, drive.ctc_max)}</strong><span>{drive.location ?? "Location to be announced"}</span><span>{drive.apply_deadline ? `Apply by ${new Date(drive.apply_deadline).toLocaleDateString("en-IN")}` : "Deadline to be announced"}</span><small>{drive.eligibility_criteria ?? "Review the role details and decide if it suits you."}</small></div></Link>) : <div className="empty-state"><strong>No published drives yet.</strong><span>New opportunities will appear here for every student as soon as the placement cell publishes them.</span></div>}</section></main>;
}